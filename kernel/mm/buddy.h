...
extern "C"
{
     ...
     * Use to get a new handle to the allocator when the arena is moved or
     * copied.
     */
    ...
    /*
     * Initializes a binary buddy memory allocator embedded in the specified
     * arena using a non-default alignment. The arena's capacity is reduced to
     * account for the allocator metadata.
     */
    ...
    /* Release a reserved memory range. Unsafe, this can mess up other
     * allocations if called with wrong parameters! */
    ...
    /*
     * Iterate through the free and allocated slots and call the provided
     * function for each of them.
     *
     * If the provided function returns a non-NULL result the iteration stops
     * and the result is returned to called. NULL is returned upon completing
...
#ifndef BUDDY_PRINTF
#define BUDDY_PRINTF(...) trace(TRACE_BUDDY, __VA_ARGS__)
#endif
...
#define INVALID_POS ((struct buddy_tree_pos){0, 0})
    ...
    static bool buddy_tree_valid(struct buddy_tree    *t,
    ...
    /* Resize the tree to the new order. When downsizing the left subtree is
     * picked. */
    ...
    static struct buddy_tree_pos buddy_tree_leftmost_child(
        struct buddy_tree *t);
    ...
    /* Returns the current sibling node position. Does not check if that is a
     * valid position */
    static inline struct buddy_tree_pos buddy_tree_sibling(
        struct buddy_tree_pos pos);

    /* Returns the parent node position or an invalid position if there is no
     * parent node */
    static inline struct buddy_tree_pos buddy_tree_parent(
        struct buddy_tree_pos pos);

    /* Returns the right adjacent node position or an invalid position if there
     * is no right adjacent node */
    ...
    /* Return the interval of the deepest positions spanning the indicated
     * position */
    ...
    static void buddy_tree_mark(struct buddy_tree    *t,
                                struct buddy_tree_pos pos);
    ...
    static void buddy_tree_debug(struct buddy_tree    *t,
                                 struct buddy_tree_pos pos, size_t start_size);
    ...
    */
    ...
    static unsigned char *address_for_position(struct buddy         *buddy,
                                               struct buddy_tree_pos pos);
    static struct buddy_tree_pos position_for_address(
        struct buddy *buddy, const unsigned char *addr);
    ...
    static struct buddy_embed_check buddy_embed_offset(size_t memory_size,
                                                       size_t alignment);
    static struct buddy_tree_pos    deepest_position_for_offset(
           struct buddy *buddy, size_t offset);
                                       ...
                                       size_t memory_size, size_t alignment)
    {
        ...
        buddy_tree_init(
            (unsigned char *)buddy + sizeof(*buddy), (uint8_t)buddy_tree_order);
                                        ...
                                        size_t alignment)
    {
        ...
        buddy = buddy_init_alignment(
            main + check_result.offset, main, check_result.offset, alignment);
        if (!buddy)
    ...
    void *buddy_malloc(struct buddy *buddy, size_t requested_size)
    {
        ...
        if (requested_size == 0)
        {
             ...
             * the particular batshit code will expect a non-NULL malloc(0)
             * result!
             *
             * See also
                        ...
                        bool ignore_data)
    {
         ...
         * - Same size as previous malloc/calloc/realloc is a no-op or a
         * rellocation
         * - Smaller size than previous *alloc decrease the allocated size with
         * an optional rellocation
         * - If the new allocation cannot be satisfied NULL is returned BUT the
         * slot is preserved
         * - Larger size than previous *alloc increase tha allocated size with
         * an optional rellocation
         */
        if (ptr == NULL)
        {
        ...
        if (!ignore_data)
        {
            ...
            memmove(
                destination,
                source,
                size_for_depth(buddy,
                               current_depth > target_depth ? current_depth
                                                            : target_depth));
        }
                             ...
                             bool ignore_data)
    {
        ...
        return buddy_realloc(
            buddy, ptr, members_count * member_size, ignore_data);
    }
                     ...
                     void *ctx)
    {
        ...
        do
        {
             ...
             *  due to a single allocation and a fully-allocated due to maxed
             * out child allocations - we need to check the children. A
             * child-allocated node will have both children set to their maximum
        ...
        struct buddy *buddy, size_t offset)
    {
        ...
        struct buddy_tree_pos pos =
            buddy_tree_leftmost_child(buddy_tree(buddy));
        pos.index += index;
        return pos;
    }
                                           ...
                                           unsigned int  state)
    {
        ...
        while (delta)
        {
            size_t current_pos_size =
                size_for_depth(buddy, buddy_tree_depth(pos));
    ...
    static bool buddy_is_free(struct buddy *buddy, size_t from)
    {
        ...
        while (buddy_tree_valid(tree, pos) &&
               (pos.index < query_range.to.index))
        {
    ...
    void buddy_debug(struct buddy *buddy)
    {
        ...
        buddy_tree_debug(buddy_tree(buddy),
                         buddy_tree_root(),
    ...
    static struct buddy_tree_pos buddy_tree_leftmost_child_internal(
        size_t tree_order);
        ...
        size_t tree_order, struct buddy_tree_pos pos)
    {
        ...
        total_offset =
            size_for_order((uint8_t)tree_order, (uint8_t)p.local_offset);
    ...
    static void buddy_tree_grow(struct buddy_tree *t, uint8_t desired_order)
    {
        ...
        while (desired_order > t->order)
        {
            ...
            pos = buddy_tree_right_child(buddy_tree_root());
            update_parent_chain(
                t, pos, buddy_tree_internal_position_tree(t, pos), 0);
        }
    }
    ...
    static void buddy_tree_shrink(struct buddy_tree *t, uint8_t desired_order)
    {
        ...
        while (desired_order < t->order)
        {
            ...
            {
                /* Get handles into the rows at the tracked depth */
                current_internal = buddy_tree_internal_position_order(
    ...
    static void write_to_internal_position(unsigned char           *bitset,
                                           ...
                                           size_t                   value)
    {
        bitset_clear_range(bitset,
                           pos.bitset_location,
                           pos.bitset_location + pos.local_offset - 1);
        if (value)
        {
            bitset_set_range(
                bitset, pos.bitset_location, pos.bitset_location + value - 1);
        }
    }
                                              ...
                                              struct internal_position pos)
    {
        ...
        return bitset_count_range(bitset,
                                  pos.bitset_location,
                                  pos.bitset_location + pos.local_offset - 1);
    }
    ...
    static void buddy_tree_mark(struct buddy_tree *t, struct buddy_tree_pos pos)
    {
        ...
        write_to_internal_position(
            buddy_tree_bits(t), internal, internal.local_offset);
    ...
    static void buddy_tree_release(struct buddy_tree    *t,
                                   struct buddy_tree_pos pos)
    {
        /* Calling release on an unused or a partially-used position a bug in
         * caller */
        struct internal_position internal =
            buddy_tree_internal_position_tree(t, pos);
                                    ...
                                    size_t                   size_current)
    {
        ...
        while (pos.index != 1)
        {
            ...
            target_parent =
                ...
                ((size_current <= size_sibling ? size_current : size_sibling) +
                 1);
                                                      ...
                                                      uint8_t target_depth)
    {
        ...
        while (current_depth != target_depth)
        {
            ...
            if (compare_with_internal_position(
                    tree_bits, left_internal, target_status + 1))
            { /* left branch is busy, pick right */
                current_pos = right_pos;
            }
            else if (compare_with_internal_position(
                         tree_bits, right_internal, target_status + 1))
            { /* right branch is busy, pick left */
                current_pos = left_pos;
            }
            else
            {
                /* One of the child nodes must be read in order to compare it to
                 * its sibling. */
                right_status =
                ...
                {
                    if (compare_with_internal_position(
                            tree_bits, left_internal, right_status))
                    {
                        current_pos = left_pos; /* Left is equal or more busy
                                                   than right, prefer left */
                    }
                    else
                    {
                        current_pos = right_pos;
    ...
    static bool buddy_tree_can_shrink(struct buddy_tree *t)
    {
        ...
        if (buddy_tree_status(t, buddy_tree_right_child(buddy_tree_root())) !=
            0)
        {
        ...
        root_value =
            read_from_internal_position(buddy_tree_bits(t), root_internal);
                                 ...
                                 struct buddy_tree_pos pos, size_t start_size)
    {
        ...
        do
        {
            ...
            BUDDY_PRINTF("%.*s",
                         ...
                         "          ");
            BUDDY_PRINTF(
                "pos index: %zu pos depth: %zu status: %zu bitset-len: %zu "
                "bitset-at: %zu",
                state.current_pos.index,
                state.current_pos.depth,
                pos_status,
                pos_internal.local_offset,
                pos_internal.bitset_location);
                                            ...
                                            struct buddy_tree_pos pos)
    {
        ...
        do
        {
            struct internal_position current_internal =
                buddy_tree_internal_position_tree(t, pos);
            size_t current_status = read_from_internal_position(
                                     ...
                                     size_t to_pos)
    {
        ...
        result =
            popcount_byte(bitset[from_bucket] &
                          bitset_char_mask[from_index][7]) +
    ...
    */
    ...
    /* Returns the highest set bit position for the given value. Returns zero
     * for zero. */
    static size_t highest_bit_position(size_t value)
    {
        size_t result = 0;
