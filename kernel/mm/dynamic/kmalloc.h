...
enum
{
    ...
    KMEM_VERBOSE_ALLOC =
        (1 << 0),  // Adds debug logging, but it butchers performance and is
                   // thus only enabled when debugging the mm
#endif
    ...
    KMEM_NO_FLAGS = (1 << 20)  // Arbitrary shift count so that this flag won't
                               // collide with others
};
