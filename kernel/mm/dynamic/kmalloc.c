...
#define KMLIST_SIZE \
    7  // Smallest slab size is 8, largest (for a small slab) 512.
static struct kmem_cache *kmlist[KMLIST_SIZE];

void kalloc_init(void)
{
    for (int i = 0, shift_offset = 3; i < KMLIST_SIZE; i++, shift_offset++)
        kmlist[i] = kmem_cache_create("kalloc", 1 << shift_offset, 0);
}
...
void *kalloc(int sz, kmem_flags_t flags)
{
    if (!SMALL_SLAB(sz))
    {
        if (flags & KMEM_PANIC)
            panic(
                "%d is too large of a size for kalloc\nMoon supports up to 512 "
                "bytes only for now",
                sz);
        return NULL;
    }
    ...
    if (flags & KMEM_PANIC)
        panic("Failed to allocate %d bytes of memory via kalloc", sz);
...
// Defined in slab.c
// This function is only needed in kalloc so it's not placed in slab.h in order
// to discourage it's usage elsewhere since it's confusingly similar to
// kmem_cache_create.
extern struct kmem_cache *kmem_cache_new(string_view name, size_t size,
                                         int alignment);

struct kmem_cache *kmem_cache_create(string_view name, size_t size,
                                     int alignment)
{
    if (!alignment)
        alignment = 8;
