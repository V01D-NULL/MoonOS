...
{
	// TODO log the timer exception to see if the kernel is getting a scheduler tick during a syscall
	/* CPU exceptions */
	if (regs->isr_number < 32)
	{
		override_quiet_boot();
		serial_set_color(BASH_RED);
		debug(true, "ERROR: %s (err_code %ld) | %lx\n", exception_messages[regs->isr_number], regs->error_code, cr2_read());
		// for(;;);
