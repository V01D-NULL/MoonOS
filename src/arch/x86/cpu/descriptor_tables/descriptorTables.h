...
/*
	...
	Access byte (GDT)
		0..3 - segment type (cs, ds, etc. )
		   4 - descriptor type?
		5..6 - privilege level
		   7 - is segment present

	Granularity byte (GDT)
		0..3 - segment length?
		   4 - 0
		   5 - 0
		   6 - operand size (0 = 16bit, 1 = 32bit)
		   7 - granularity? (0 = 1byte, 1 = 1kbyte)
...
// Pointer to an array of GDT entries.
// This is in a format suitable for giving to 'lgdt'.
